          ...
          <Table
            ...
            columns={[
                ...
                onCell: () => ({ style: { textAlign: 'left' } }), },
              {
                ...
                render: (_: unknown, r: TechInvestment) => r.equipment.reduce((s, i) => s + i.subtotal, 0).toFixed(2),
              },
              {
                title: '材料费用(万元)',
                key: 'materials',
                onHeaderCell: () => ({ style: { textAlign: 'right' } }),
                onCell: () => ({ style: { textAlign: 'right' } }),
                render: (_: unknown, r: TechInvestment) => r.materials.reduce((s, i) => s + i.subtotal, 0).toFixed(2),
              },
              {
                title: '安装费用(万元)',
                key: 'installation',
                onHeaderCell: () => ({ style: { textAlign: 'right' } }),
                onCell: () => ({ style: { textAlign: 'right' } }),
                render: (_: unknown, r: TechInvestment) => r.installation.reduce((s, i) => s + i.subtotal, 0).toFixed(2),
              },
              {
                title: '运维费用(万元)',
                key: 'maintenance',
                onHeaderCell: () => ({ style: { textAlign: 'right' } }),
                onCell: () => ({ style: { textAlign: 'right' } }),
                render: (_: unknown, r: TechInvestment) => r.maintenance.reduce((s, i) => s + i.subtotal, 0).toFixed(2),
              },
              {
                title: '固定投资(万元)',
