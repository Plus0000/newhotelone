...
} from '@/steps/step4-energy/components/helpers';
...
function getEnergySavingTotal(step4: Step4ProjectData | undefined): {
  ...
  const techs = Object.values(step4.techs).map(migrateTechEnergyByType);
  return {
    originalEnergy: techs.reduce((s, t) => s + (t.originalEnergyRun || 0), 0),
    savingEnergy: techs.reduce((s, t) => s + (t.savingEnergyRun || 0), 0),
    originalEnergyByType: techs.reduce(
      (acc, t) => ({
        electric: acc.electric + t.originalEnergyByType.electric,
        gas: acc.gas + t.originalEnergyByType.gas,
        heat: acc.heat + t.originalEnergyByType.heat,
      }),
      { electric: 0, gas: 0, heat: 0 },
    ),
    savingEnergyByType: techs.reduce(
      (acc, t) => ({
        electric: acc.electric + t.savingEnergyByType.electric,
        gas: acc.gas + t.savingEnergyByType.gas,
        heat: acc.heat + t.savingEnergyByType.heat,
      }),
      { electric: 0, gas: 0, heat: 0 },
    ),
...
export default function ReportView({
  ...
  const totalInvestment =
    (decisionData.totalFixedInvestment || 0) +
  ...
  const carbonSaving = calcCarbonSavingByType(
    energy.originalEnergyByType,
    energy.savingEnergyByType,
    province || undefined,
  );
  ...
  const overallScore = useMemo(() => {
    ...
    const weighted =
      efficiencyScore * 0.3 + economicScore * 0.4 + policyScore * 0.2 + feasibleScore * 0.1;
    return {
      efficiencyScore,
      economicScore,
      policyScore,
  ...
  const analysisTexts = useMemo(() => {
    ...
    const techData = selectedTechIds.map((id) => {
      ...
      const t4Raw = step4Data?.techs?.[id];
      const t4 = t4Raw ? migrateTechEnergyByType(t4Raw) : undefined;
      ...
      const originalEnergyByType = t4?.originalEnergyByType ?? {
        electric: 0,
        gas: 0,
        heat: 0,
      };
      const savingEnergyByType = t4?.savingEnergyByType ?? { electric: 0, gas: 0, heat: 0 };
      const carbon = calcCarbonSavingByType(originalEnergyByType, savingEnergyByType, province);
      return {
        id,
        name: tech?.name || id,
        ...
        savingEnergyByType,
        energySavingRate: tech?.energySavingRate || '',
        originalCost: t4?.originalCostRun || 0,
        savingCost: t4?.savingCostRun || 0,
      };
    });

    // Sort by payback for analysis
    const byPayback = [...techData].sort(
      (a, b) => a.totalInv / Math.max(a.annualSaving, 1) - b.totalInv / Math.max(b.annualSaving, 1),
    );
    const byCoalPerInv = [...techData].sort(
      (a, b) => b.coal / Math.max(b.totalInv, 1) - a.coal / Math.max(a.totalInv, 1),
    );
    const byAreaBenefit = [...techData].sort(
      (a, b) => b.annualSaving / totalArea - a.annualSaving / totalArea,
    );
    ...
    return {
      ...
      totalCarbon: techData.reduce((s, t) => s + t.carbon, 0),
    };
  }, [selectedTechIds, techMap, techInvestments, step4Data, totalArea, province]);
  ...
  const summaryCols: ColumnsType<CalcSummaryRow> = [
    {
      ...
      key: 'seq',
      width: 50,
      onCell: () => ({ style: tdCStyle }),
      onHeaderCell: () => ({ style: thStyle }),
    },
    {
      title: '指标名称',
      dataIndex: 'name',
      key: 'name',
      width: 200,
      onCell: () => ({ style: tdLStyle }),
      onHeaderCell: () => ({ style: thLStyle }),
    },
    {
      title: '单位',
      dataIndex: 'unit',
    ...
    },
    {
      ...
      key: 'value',
      width: 100,
      onCell: () => ({ style: tdRStyle }),
      onHeaderCell: () => ({ style: thRStyle }),
    ...
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
      ...
      >
        ...
        <div
          ...
          style={{ textAlign: 'center', padding: '60px 0 40px', pageBreakAfter: 'always' }}
        >
          ...
          />
          <div style={{ fontSize: 16, color: '#434343', marginBottom: 4, marginTop: 8 }}>
            {project.projectName}
          </div>
          <div style={{ fontSize: 15, color: '#595959', marginBottom: 32 }}>
            {project.hospitalName}
          </div>
          <div style={{ fontSize: 14, color: '#8c8c8c', lineHeight: 2 }}>
            <div>
              报告编号：JN-{new Date().getFullYear()}-
            ...
            <tbody>
              {(
                [
                  ...
                  [
                    '床位数',
                    step1Data?.normalBeds
                      ? `${(step1Data.normalBeds as number) + ((step1Data?.icuBeds as number) || 0)}张`
                      : '—',
                  ],
                  [
                    '日门诊量',
                    step1Data?.dailyOutpatient ? `${step1Data.dailyOutpatient}人次` : '—',
                  ],
                  [
                    '气候分区',
                    step1Data?.climateZone ? (step1Data.climateZone as string[]).join('、') : '—',
                  ],
                  [
                    '现有冷热源系统',
                    step1Data?.mep
                      ? `${(step1Data.mep as Record<string, string>)?.coldSource || ''}/${(step1Data.mep as Record<string, string>)?.heatSource || ''}`
                      : '—',
                  ],
                  [
                    '现有照明系统',
                    step1Data?.mep
                      ? [
                          ...((step1Data.mep as Record<string, string[]>)?.lightingNonEnergy || []),
                          ...((step1Data.mep as Record<string, string[]>)?.lightingEnergy || []),
                        ].join('、') || '—'
                      : '—',
                  ],
                ] as [string, string][]
          ...
          {hasStep4 ? (
            <>
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 12 }}>
                ...
                <tbody>
                  {[
                    {
                      type: '电力',
                      consumption: fmt(energy.originalEnergyByType.electric, 2),
                      unit: '万kWh/a',
                      cost: energy.originalCost,
                      pct: energy.originalCost > 0 ? '—' : '—',
                    },
                    {
                      type: '天然气',
                      consumption:
                        energy.originalEnergyByType.gas > 0
                          ? fmt(energy.originalEnergyByType.gas, 2)
                          : '—',
                      unit: '万Nm³/a',
                      cost: '—',
                      pct: '—',
                    },
                    {
                      type: '热力',
                      consumption:
                        energy.originalEnergyByType.heat > 0
                          ? fmt(energy.originalEnergyByType.heat, 2)
                          : '—',
                      unit: 'GJ/a',
                      cost: '—',
                      pct: '—',
                    ...
                    { type: '自来水', consumption: '—', unit: '万m³/a', cost: '—', pct: '—' },
                    {
                      ...
                      consumption: '—',
                      unit: '—',
                      cost: fmt(energy.originalCost, 2),
                      pct: '100%',
                    },
                  ].map((r) => (
                    <tr key={r.type}>
                      <td style={{ ...tdLStyle, fontWeight: r.type === '合计' ? 600 : 400 }}>
                        {r.type}
                      </td>
              ...
              <div style={infoBoxStyle}>
                * 电力/天然气/热力消耗量基于系统测算，综合年运行费用为{' '}
                {fmt(energy.originalCost, 2)} 万元。自来水数据需补充详细计量。
              </div>
            </>
          ) : (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 12 }}>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td style={tdLStyle}>
                  {energyPrices?.gasPrice
                    ? `${fmt(energyPrices.gasPrice, 2)}元/Nm³`
                    : '按当地天然气价格执行'}
                </td>
              </tr>
              <tr>
        ...
        <div className="report-section" style={{ marginBottom: 28, pageBreakBefore: 'always' }}>
          ...
          {!hasTechs ? (
            ...
            <>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 12 }}>
                ...
                <tbody>
                  {selectedTechIds.map((id, idx) => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td style={tdCStyle}>
                          {tech ? CATEGORY_LABEL[tech.category] || tech.category : '—'}
                        </td>
                  ...
                  <tr style={{ background: '#fafafa', fontWeight: 600 }}>
                    ...
                    <td style={tdCStyle}>
                      {analysisTexts
                        ? fmtPct((annualCostSaving / energy.originalCost) * 100, 1)
                        : '—'}
                    </td>
              ...
              >
                本方案共推荐 {selectedTechIds.length}{' '}
                项节能技术，覆盖供热、制冷、照明三大核心能耗系统， 综合节能率{' '}
                {fmtPct((annualCostSaving / energy.originalCost) * 100, 1)}， 年节约能源费用{' '}
                {fmt(annualCostSaving, 2)} 万元， 静态投资回收期约 {fmt(cr.staticPayback, 1)} 年。
              </div>
            </>
          )}
        </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 14 }}>
                    ...
                    <tbody>
                      {[
                        ...
                        <tr key={row.label}>
                          <td style={{ ...tdLStyle, fontWeight: row.bold ? 600 : 400 }}>
                            {row.label}
                          </td>
                      ...
                      {[
                        {
                          ...
                          save: saveCost,
                          orig: origCost,
                          diff: saveCost - origCost,
                        },
                        { label: '年运维费用', save: maint10Y, orig: 0, diff: maint10Y },
                        {
                          label: '年运行费用总计',
                          save: saveCost + maint10Y,
                          orig: origCost,
                          diff: saveCost + maint10Y - origCost,
                          bold: true,
                        },
                      ].map((row) => (
                        <tr key={row.label}>
                          ...
                          <td
                            style={{
                              ...tdRStyle,
                              color: row.diff < 0 ? '#52c41a' : '#ff4d4f',
        ...
        <div className="report-section" style={{ marginBottom: 28, pageBreakBefore: 'always' }}>
          ...
          {!analysisTexts ? (
            ...
            <>
              ...
              <AnalysisConclusion
                text={`单位投资节能效率最高的技术是${analysisTexts.byCoalPerInv[0]?.name || '—'}和${analysisTexts.byCoalPerInv[1]?.name || '—'}，是优先落地的高性价比技术。`}
              />
                ...
                <tbody>
                  {analysisTexts.byAreaBenefit.map((t, i) => (
                    <tr key={t.id}>
                      ...
                      <td style={tdRStyle}>
                        {totalArea > 0 ? fmt((t.annualSaving / totalArea) * 10000, 1) : '—'}
                      </td>
                      <td style={tdCStyle}>{i + 1}</td>
                    </tr>
                  ))}
              ...
              <AnalysisConclusion
                text={`${analysisTexts.byAreaBenefit[0]?.name || '—'}和${analysisTexts.byAreaBenefit[1]?.name || '—'}的单位面积节能收益最高，适合在大规模改造项目中应用。`}
              />
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
                ...
                <tbody>
                  {analysisTexts.byPayback.map((t) => {
                    ...
                    const priority =
                      payback <= 3 ? '极高' : payback <= 5 ? '高' : payback <= 10 ? '中' : '低';
                    return (
                      <tr key={t.id}>
                        <td style={tdLStyle}>{t.name}</td>
                        <td style={tdRStyle}>{fmt(t.totalInv, 2)}</td>
                        ...
                        <td style={{ ...tdCStyle, fontWeight: priority === '极高' ? 600 : 400 }}>
                          {priority}
                        </td>
                      </tr>
                    );
              ...
              <AnalysisConclusion
                text={`${analysisTexts.byPayback[0]?.name || '—'}的投资回收期最短，属于短期快回本技术，可优先批量推广。`}
              />
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
                ...
                <tbody>
                  {[...analysisTexts.techData]
                    .sort(
                      (a, b) =>
                        b.annualSaving / Math.max(b.totalInv, 1) -
                        a.annualSaving / Math.max(a.totalInv, 1),
                    )
                    .map((t, i) => (
                      <tr key={t.id}>
                        <td style={tdLStyle}>{t.name}</td>
                        ...
                        <td style={tdRStyle}>{fmt(t.totalInv, 2)}</td>
                        <td style={tdRStyle}>
                          {t.totalInv > 0 ? fmtPct((t.annualSaving / t.totalInv) * 100, 1) : '—'}
                        </td>
                        <td style={tdCStyle}>{i + 1}</td>
                      </tr>
                    ))}
              ...
              <AnalysisConclusion
                text={`结合静态投资回收期分析，投资回收期短年均净收益率高的技术适合快速落地，回收期较长的技术适合作为长期智慧化升级项目。`}
              />
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
                ...
                <tbody>
                  {analysisTexts.techData
                    .sort(
                      (a, b) =>
                        a.annualMaint / Math.max(a.annualSaving, 1) -
                        b.annualMaint / Math.max(b.annualSaving, 1),
                    )
                    .map((t) => {
                      const ratio = t.annualSaving > 0 ? t.annualMaint / t.annualSaving : 1;
                      const level =
                        ratio <= 0.05 ? '极低' : ratio <= 0.15 ? '低' : ratio <= 0.3 ? '中' : '高';
                      return (
                        <tr key={t.id}>
                          <td style={tdLStyle}>{t.name}</td>
              ...
              <AnalysisConclusion
                text={`运维成本占比低的技术收益稳定性更高，高占比技术需在合同中明确运维责任划分。`}
              />
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
                ...
                <tbody>
                  {[
                    {
                      label: '能源价格上涨20%',
                      cost: annualCostSaving * 1.2,
                      payback: totalInvestment / (annualCostSaving * 1.2 || 1),
                      change: '+15.9%',
                    },
                    {
                      label: '能源价格上涨10%',
                      cost: annualCostSaving * 1.1,
                      payback: totalInvestment / (annualCostSaving * 1.1 || 1),
                      change: '+7.9%',
                    },
                    {
                      label: '基准价格',
                      cost: annualCostSaving,
                      payback: cr.staticPayback,
                      change: '0%',
                    },
                    {
                      label: '能源价格下降10%',
                      cost: annualCostSaving * 0.9,
                      payback: totalInvestment / (annualCostSaving * 0.9 || 1),
                      change: '-7.9%',
                    },
                    {
                      label: '能源价格下降20%',
                      cost: annualCostSaving * 0.8,
                      payback: totalInvestment / (annualCostSaving * 0.8 || 1),
                      change: '-15.9%',
                    },
                  ].map((r) => (
                    <tr key={r.label}>
              ...
              <AnalysisConclusion
                text={`项目收益对能源价格变动较为敏感，能源价格每上涨10%，综合投资回收期缩短约${fmt(cr.staticPayback - totalInvestment / (annualCostSaving * 1.1 || 1), 1)}年，整体抗风险能力较强。`}
              />
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
                ...
                <tbody>
                  {analysisTexts.techData
                    ...
                    .map((t) => {
                      const carbon = t.carbon;
                      const totalCarbon = analysisTexts.totalCarbon;
                      const gasSaving =
                        t.originalEnergyByType.gas - t.savingEnergyByType.gas;
                      return (
                        <tr key={t.id}>
                          <td style={tdLStyle}>{t.name}</td>
                          <td style={tdRStyle}>{fmt(t.annualEnergySaving, 2)}</td>
                          <td style={tdCStyle}>
                            {gasSaving > 0 ? fmt(gasSaving, 2) : '—'}
                          </td>
                          <td style={tdRStyle}>{fmt(carbon, 2)}</td>
                          <td style={tdCStyle}>
                            {totalCarbon > 0 ? fmtPct((carbon / totalCarbon) * 100, 1) : '—'}
                          </td>
                        </tr>
                      );
                    })}
              ...
              <AnalysisConclusion
                text={`项目年节碳量合计 ${fmt(carbonSaving, 0)} tCO₂，符合国家双碳政策导向，可申报相关节能补贴和碳交易额度。`}
              />
            </>
          )}
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 16 }}>
            <tbody>
              {[
                ...
                {
                  label: '节能收益分成比例',
                  value: `甲方${fmt(decisionData.initialProfitShare1 || 30, 0)}%，乙方${fmt(decisionData.initialProfitShare2 || 70, 0)}%`,
                },
                {
                  label: '总投资',
                  value: `${fmt(totalInvestment || (cr.summary.find((r) => r.seq === '1')?.value as number) || 0, 2)} 万元`,
                },
                {
                  label: '银行贷款比例',
                  value: decisionData.fundingRatio
                    ? `${fmt(100 - decisionData.fundingRatio, 0)}%`
                    : '—',
                },
                {
                  label: '贷款利率',
                  value: decisionData.loanRate ? `${fmt(decisionData.loanRate, 2)}%` : '—',
                },
                {
                  label: '贷款期限',
                  value: decisionData.repaymentPeriod ? `${decisionData.repaymentPeriod} 年` : '—',
                },
                { label: '折现率', value: '8%' },
              ].map((r) => (
                <tr key={r.label}>
                  <td style={{ ...tdLStyle, width: 200, background: '#fafafa', fontWeight: 500 }}>
                    {r.label}
                  </td>
                ...
                <tbody>
                  {cr.totalCost.map((r) => (
                    <tr key={r.year}>
                      ...
                      <td style={tdRStyle}>
                        {fmt(r.maintenanceCost + r.laborCost + r.adminCost + r.insuranceCost, 2)}
                      </td>
                ...
                <tbody>
                  {cr.profit.map((r) => (
                    <tr key={r.year}>
                      ...
                      <td
                        style={{ ...tdRStyle, color: r.profitTotal >= 0 ? '#1a1a1a' : '#ff4d4f' }}
                      >
                        {fmt(r.profitTotal, 2)}
                      </td>
                      ...
                      <td
                        style={{
                          ...tdRStyle,
                          fontWeight: 600,
                          color: r.netProfit >= 0 ? '#1a1a1a' : '#ff4d4f',
                        }}
                      >
                        {fmt(r.netProfit, 2)}
                      </td>
                ...
                <tbody>
                  {cr.projectCashflow.map((r) => (
                    <tr key={r.year}>
                      ...
                      <td
                        style={{ ...tdRStyle, color: r.netCashflow >= 0 ? '#1a1a1a' : '#ff4d4f' }}
                      >
                        {fmt(r.netCashflow, 2)}
                      </td>
                      <td
                        style={{
                          ...tdRStyle,
                          color: r.cumulativeDiscounted >= 0 ? '#52c41a' : '#ff4d4f',
                        }}
                      >
                        {fmt(r.cumulativeDiscounted, 2)}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="report-section" style={{ marginBottom: 28, pageBreakBefore: 'always' }}>
          ...
          <div
            style={{
              ...
              fontWeight: 600,
              color:
                overallScore.total >= 85
                  ? '#52c41a'
                  : overallScore.total >= 70
                    ? '#1677ff'
                    : overallScore.total >= 55
                      ? '#fa8c16'
                      : '#ff4d4f',
              marginBottom: 16,
            }}
          ...
          {hasTechs && analysisTexts && (
            <>
              <div style={{ marginBottom: 12 }}>
                ...
                {analysisTexts.byPayback
                  .filter((t) => t.annualSaving > 0 && t.totalInv / t.annualSaving <= 5)
                  .slice(0, 2)
                  .map((t) => {
                    ...
                    return (
                      ...
                      >
                        实施 {t.name}，投资 {fmt(t.totalInv, 2)} 万元，年节约费用{' '}
                        {fmt(t.annualSaving, 2)} 万元，投资回收期 {fmt(payback, 1)} 年
                      </div>
                    );
                  })}
              </div>
          ...
          >
            <div>
              • <Text strong>资金充裕型医院：</Text>建议采用自投模式，独享全部节能收益，投资回收期{' '}
              {fmt(cr.staticPayback, 1)} 年
            </div>
            <div>
              • <Text strong>资金紧张型医院：</Text>
              建议采用EMC模式，零投资、零风险，合作期内可获得稳定节能收益，合作期满后所有设备无偿归医院所有
            </div>
            <div>
              • <Text strong>新建医院：</Text>
              建议采用BOT模式，由投资方负责投资、建设、运营，合作期满后移交医院
            </div>
          </div>
          ...
          >
            <div>
              • 积极申报当地节能改造补贴，预计可获得补贴约 {fmt((carbonSaving * 300) / 10000, 0)}{' '}
              万元
            </div>
